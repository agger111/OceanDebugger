...
if (-not $isAdmin) {
    ...
    if ($scriptPath) {
        # Running from a saved .ps1 file on disk -> relaunch that same file elevated.
        ...
        # Running via "iwr | iex" (no file on disk) -> can't reliably self-relaunch.
        # Ask the user to elevate first, then re-run the same command.
        Write-Host "===============================================================================" -ForegroundColor Red
        Write-Host " This tool needs to run as Administrator." -ForegroundColor Red
        Write-Host " Close this window, open PowerShell as Administrator (Start -> type PowerShell" -ForegroundColor Yellow
        Write-Host " -> right-click -> Run as administrator), then paste the same command again." -ForegroundColor Yellow
        Write-Host "===============================================================================" -ForegroundColor Red
        pause
        exit
    }
...
# Force TLS 1.2 (and 1.3 if this .NET/Windows version supports it) in ServicePointManager.
# Tls13 doesn't exist as an enum member on older .NET Framework builds - if we don't
# guard this, a failed assignment here can silently leave TLS 1.2 unset too, which
# then causes every handshake test below to fail for reasons unrelated to Ocean.
try {
...
# -----------------------------------------------------------------------------
# SETUP: Ocean-Debug-Logs folder
# -----------------------------------------------------------------------------
# Use a fixed, always-valid absolute location rather than "next to the script" -
# when the script is run via "iwr | iex" there IS no script file on disk, so
# $PSScriptRoot is empty and any path built from it breaks (this caused the
# "opens This PC with nothing" issue). Desktop always exists and is writable.
$DebugFolder = Join-Path -Path ([Environment]::GetFolderPath("Desktop")) -ChildPath "Ocean-Debug-Logs"
...
$endpointResults   = @{}   # url -> $true/$false, used to avoid false-positive CRITICAL verdicts
...
foreach ($url in $targetEndpoints) {
    ...
    $req.Timeout = 8000
    $req.UserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) OceanAC-DiagnosticHandshake/1.0"
    $req.Accept = "*/*"
    $req.Method = "GET"
    ...
    try {
        ...
        $endpointResults[$url] = $true
    } catch [System.Net.WebException] {
        $ex = $_.Exception
        if ($ex.Response) {
            # A TLS handshake had to succeed for the server to send back ANY HTTP
            # response, even an error one (401/403/404/etc). So this still counts
            # as a working connection - many API endpoints reject generic probe
            # requests like this on purpose without it meaning anything is broken.
            ...
            $endpointResults[$url] = $false
 
            if ($errorMsg -like "*NameResolution*") {
            ...
            } elseif ($errorMsg -like "*SSL/TLS*" -or $errorMsg -like "*secure channel*") {
                Log-Output "   -> DIAGNOSIS: This probe's TLS handshake failed for $url." -Status "WARN"
            }
        }
    } catch {
...
# Only escalate to a CRITICAL "system-wide TLS interception" verdict if EVERY
# endpoint failed. If ocean.ac connects fine but one specific subdomain (e.g. an
# API-only endpoint) rejects this generic probe, that's most likely the endpoint
# filtering non-client requests (bot/User-Agent/method checks) - not a broken or
# intercepted TLS stack - and Ocean's own client can still connect normally.
$failedEndpoints = $endpointResults.GetEnumerator() | Where-Object { $_.Value -eq $false }
if ($failedEndpoints -and ($failedEndpoints.Count -lt $targetEndpoints.Count)) {
    Log-Output "NOTE: $($failedEndpoints.Count) of $($targetEndpoints.Count) probed endpoint(s) rejected this generic diagnostic request, but at least one Ocean endpoint connected successfully. This does NOT necessarily mean Ocean AC itself is blocked - some endpoints only accept requests from the real game client." -Status "INFO"
} elseif ($failedEndpoints -and $failedEndpoints.Count -eq $targetEndpoints.Count) {
    Log-Output "ALL probed Ocean endpoints failed to connect. This points to a genuine local network/TLS interception issue." -Status "CRITICAL"
...
# -----------------------------------------------------------------------------
# PHASE 4.5: PLAYER INFO LOG
# -----------------------------------------------------------------------------
# Only ever records info about the person running the tool on THIS machine.
# Minecraft username is read from the local launcher profile (just a display
# name, not a credential). Discord username is typed in by hand - Discord
# doesn't expose "just the username" anywhere outside its local storage
# database, which is the same place session tokens live, so we don't touch it.
Write-Host "`n-------------------------------------------------------------------------------" -ForegroundColor Gray
Write-Host " [PHASE 4.5] PLAYER INFO..." -ForegroundColor Yellow
Write-Host "-------------------------------------------------------------------------------" -ForegroundColor Gray
 
$PlayerFile = Join-Path -Path $DebugFolder -ChildPath "Player_$Timestamp.txt"
 
# Try to read the last-used Minecraft profile name from the official launcher.
$mcUsername = $null
$launcherProfilesPath = "$env:APPDATA\.minecraft\launcher_profiles.json"
if (Test-Path $launcherProfilesPath) {
    try {
        $profiles = Get-Content $launcherProfilesPath -Raw | ConvertFrom-Json
        if ($profiles.authenticationDatabase) {
            $account = $profiles.authenticationDatabase.PSObject.Properties | Select-Object -First 1
            if ($account) { $mcUsername = $account.Value.displayName }
        }
    } catch {}
}
if (-not $mcUsername) {
    $mcUsername = Read-Host "Could not auto-detect a Minecraft username. Enter it manually (or press Enter to skip)"
}
Log-Output "Minecraft Username: $mcUsername" -Status "INFO"
 
# Discord username - typed in, never read from Discord's local storage/cache.
$discordUsername = Read-Host "Enter your Discord username (or press Enter to skip)"
Log-Output "Discord Username: $discordUsername" -Status "INFO"
 
"Minecraft Username: $mcUsername`nDiscord Username: $discordUsername`nCaptured: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Set-Content -Path $PlayerFile -Encoding UTF8
Log-Output "Saved player info to: $PlayerFile" -Status "INFO"
